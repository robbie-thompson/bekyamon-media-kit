...
After deployment, visit `/admin`, log in, review the starter values, and click **Save changes**. The first save writes the starter data and default layout to KV. The public API can use the bundled starter data before this initial save, so the default media kit remains available during setup.
...
Wrangler provides a local KV store. Open the URL printed by Wrangler, then visit `/admin`.
...
- Content images accept either a filename under `public/images` or an absolute HTTPS URL.
