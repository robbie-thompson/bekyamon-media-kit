import { CONTENT_IMAGES } from './default-data.js';
...
<body>
  ...
  (() => {
    ...
    const availableImages = ${JSON.stringify(CONTENT_IMAGES)};
    let state = null;
    ...
    function imageSelect(value, path) { return '<select aria-label="Image" data-path="' + escapeAttr(path) + '">' + availableImages.map((image) => '<option value="' + escapeAttr(image.value) + '" ' + (image.value === value ? 'selected' : '') + '>' + escapeAttr(image.label) + '</option>').join('') + '</select>'; }
